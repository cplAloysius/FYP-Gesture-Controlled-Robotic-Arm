...
class identify_GetTarget:
    ...
    def follow_function(self, img, HSV_config):
        ...
        if len(cnts) > 0:
            ...
            if color_radius > 10:
                # Mark the detected color with the prototype coi
                # 将检测到的颜色用原形线圈标记出来
                cv.circle(self.img, (int(color_x), int(color_y)), int(color_radius), (255, 0, 255), 3)

                scale_x, scale_y = self.scaling(color_x, color_y)
                
                (a, b) = (round((((color_x*scale_x) - 320) / 4000), 5), round(((480 - (color_y*scale_y)) / 3000) * 0.8+0.19, 5))
    ...
    def target_run(self, cntr, xy=None):
        '''
        grab function 抓取函数
        ...
        #move_status=0
        if cntr == None and self.grap.holding == 0:
            return
        else: 
            self.move_status = 1
        ...
        try:
            ...
            # 此处ROS反解通讯,获取各关节旋转角
            if self.grap.holding == 0:
                joints = self.server_joint(cntr)
                self.old_joints = joints
                if joints[0] > 180 or joints[0] < 0:
                    self.exceed_angle = 1
                    return
            else:
                self.old_joints[0] = self.xy[0]
                joints = self.old_joints
    ...
    def server_joint(self, posxy):
        ...
        try:
            ...
            if isinstance(response, kinemaricsResponse):
                ...
                offset = 90 - response.joint1
                joints[0] = self.xy[0] - offset
