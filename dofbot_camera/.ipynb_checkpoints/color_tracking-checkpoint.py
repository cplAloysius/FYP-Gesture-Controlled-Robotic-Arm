...
class color_follow:
    def __init__(self):
        ...
        self.target_servoz=110
        self.Arm = Arm_Lib.Arm_Device()
        # self.xservo_pid = PID.PositionalPID(1.9, 0.3, 0.35)
        # self.yservo_pid = PID.PositionalPID(1.9, 0.3, 0.35)
        self.xservo_pid = PID.PositionalPID(0.9, 0.15, 0.7)
        self.yservo_pid = PID.PositionalPID(0.9, 0.15, 0.7)
        self.zservo_pid = PID.PositionalPID(0.9, 0.15, 0.7)
        self.reps = 0
    ...
    def follow_function(self, img, HSV_config):
        ...
        if len(cnts) > 0:
            ...
            if color_radius > 10:
                ...
                cv.putText(self.img, str(round(color_radius)), (300, 30), cv.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
                cv.putText(self.img, str(self.target_servoz), (100, 30), cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
#                 cv.putText(self.img, follow_name, (300, 30), cv.FONT_HERSHEY_SIMPLEX, 0.8, (105, 105, 105), 2)
                ...
                if (self.reps >= 100):
                    self.zservo_pid.SystemOutput = color_radius
                    self.zservo_pid.SetStepSignal(100)
                    self.zservo_pid.SetInertiaTime(0.01, 0.1)
                    target_valuez = 180 - int(1500 + self.zservo_pid.SystemOutput)
                    self.target_servoz = int((target_valuez - 500) / 10)
                    cv.putText(self.img, str(round(self.zservo_pid.ErrBack)), (20, 30), cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                    if self.target_servoz > 180:self.target_servoz = 180
                    if self.target_servoz < 0: self.target_servoz = 0

                self.reps += 1
                joints_0 = [self.target_servox, self.target_servoz, self.target_servoy/2, self.target_servoy/2, 90, 30]
